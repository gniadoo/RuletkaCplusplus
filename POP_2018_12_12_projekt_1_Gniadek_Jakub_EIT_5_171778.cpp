//POP_2017_12_12_projekt_1_Gniadek_Jakub_EIT_5_171778
//DEV-C++ 5.11 TDM-GCC 4.9.2

/* GRA RULETKA DLA JEDNEGO DO CZTERECH GRACZY */
...
void funkcja_zerujaca(char tab1[] ,char splitx1[][n] ,char splitx2[][n] , char cornerx[], char zaklady_specjalnex[]){ //przywraca stó³ do stanu pierwotnego
...
void rysuj(char straightx[], char splitx1[][n] ,char splitx2[][n] , char cornerx[], char zaklady_specjalnex[]){ //rysowanie pola do gry z uwzglêdnieniem postawionych zak³adów
cout<<" "<<endl;
cout<<"  +-----+-----+-----+-----+-----+-----+-----+-----+-----+-----+-----+-----+-----+"<<endl;
...
void czy_poprawne_zetony(zaklad zakl[][max_graczy][max_zakladow], int i, int j, int ktora_runda, int punkty[]){ //sprawdzenie czy gracz ma odpowiednia ilosc zetonow ktore chce postawic
	cout<<"Zostalo ci "<<punkty[i]<<" zetonow"<<endl;
	cout<<"Ile chcesz postawic?"<<endl;
	bool poprawne_zetony=false; 
		
		while(poprawne_zetony==false){
			cin>>zakl[ktora_runda][i][j].kasa;
				if(zakl[ktora_runda][i][j].kasa<0 || zakl[ktora_runda][i][j].kasa>punkty[i]){
	...
	punkty[i]=punkty[i]-zakl[ktora_runda][i][j].kasa;	
}

void obstaw_zaklad1( zaklad zakl[][max_graczy][max_zakladow], int j, int i, int ktora_runda, int punkty[] ,char straightx[]){ //obstawianie zakladu typu Straight Up
	...
	zakl[ktora_runda][i][j].nr_zak=1; //wpisanie informacji o obstawionym zak³adzie do tablicy struktur ¿eby pozniej mo¿na by³o srawdziæ czy gracz wygra³
	zakl[ktora_runda][i][j].tabp[0]=x;//
			
	czy_poprawne_zetony(zakl, i, j, ktora_runda, punkty);	
	
	straightx[x-1]='X'; //zaznaczenie X zakladu obstawionego przez gracza na stole
}
	
void wynik_zaklad_typ1(zaklad zakl[][max_graczy][max_zakladow], int i, int j, int liczba, int ktora_runda, int punkty[] ){ //sprawdzanie czy liczba obstawiona przez gracza w zakladzie Straight Up zosta³a wylosowana
	
	if(zakl[ktora_runda][i][j].tabp[0]==liczba){
		punkty[i]=punkty[i]+36*zakl[ktora_runda][i][j].kasa;
	}	
}	
	

void obstaw_zaklad2( zaklad zakl[][max_graczy][max_zakladow], int j, int i, int ktora_runda, int punkty[] ,char splitx1[][n] ,char splitx2[][n]){ //obstawianie zakladu typu Split Bet
	...
	zakl[ktora_runda][i][j].nr_zak=2;    //
	zakl[ktora_runda][i][j].tabp[0]=l1;  // wpisanie informacji o obstawionym zak³adzie do tablicy struktur ¿eby pozniej mo¿na by³o srawdziæ czy gracz wygra³
	zakl[ktora_runda][i][j].tabp[1]=l2;  //
...
void wynik_zaklad_typ2(zaklad zakl[][max_graczy][max_zakladow], int i, int j, int liczba, int ktora_runda, int punkty[] ){ //sprawdzanie czy liczba obstawiona przez gracza w zakladzie Split Bet zosta³a wylosowana
	if(zakl[ktora_runda][i][j].tabp[0]==liczba || zakl[ktora_runda][i][j].tabp[1]==liczba){
					punkty[i]=punkty[i]+18*zakl[ktora_runda][i][j].kasa;
	}
}		
	
void obstaw_zaklad3(zaklad zakl[][max_graczy][max_zakladow], int j, int i, int ktora_runda, int punkty[], char cornerx[] ){ //obstawianie zakladu typu Corner Bet
	...
	zakl[ktora_runda][i][j].nr_zak=3;    //
	zakl[ktora_runda][i][j].tabp[0]=x;   //
	zakl[ktora_runda][i][j].tabp[1]=x+3; // wpisanie informacji o obstawionym zak³adzie do tablicy struktur ¿eby pozniej mo¿na by³o srawdziæ czy gracz wygra³
	zakl[ktora_runda][i][j].tabp[2]=x-1; //
	zakl[ktora_runda][i][j].tabp[3]=x+2; //
...
void wynik_zaklad_typ3(zaklad zakl[][max_graczy][max_zakladow], int i, int j, int liczba, int ktora_runda, int punkty[] ){ //sprawdzanie czy liczba obstawiona przez gracza w zakladzie Corner Bet zosta³a wylosowana
	if(zakl[ktora_runda][i][j].tabp[0]==liczba){
		punkty[i]=punkty[i]+9*zakl[ktora_runda][i][j].kasa;
	}
	else if(zakl[ktora_runda][i][j].tabp[1]==liczba){
		punkty[i]=punkty[i]+9*zakl[ktora_runda][i][j].kasa;
	}
	else if(zakl[ktora_runda][i][j].tabp[2]==liczba){
		punkty[i]=punkty[i]+9*zakl[ktora_runda][i][j].kasa;
	}
	else if(zakl[ktora_runda][i][j].tabp[3]==liczba){
		punkty[i]=punkty[i]+9*zakl[ktora_runda][i][j].kasa;
	}
}	

void obstaw_zaklad4( zaklad zakl[][max_graczy][max_zakladow], int j, int i, int ktora_runda, int punkty[], char zaklady_specjalnex[]){ //obstawianie zakladu typu Dozen Bet
	...
	zakl[ktora_runda][i][j].nr_zak=4; //wpisanie informacji o obstawionym zak³adzie do tablicy struktur ¿eby pozniej mo¿na by³o srawdziæ czy gracz wygra³
	zakl[ktora_runda][i][j].tabp[0]=x; //
...
void wynik_zaklad_typ4(zaklad zakl[][max_graczy][max_zakladow], int i, int j, int liczba, int ktora_runda, int punkty[] ){ //sprawdzanie czy liczba obstawiona przez gracza w zakladzie Dozen Bet zosta³a wylosowana
	if(zakl[ktora_runda][i][j].tabp[0]==1){
		if(liczba>=1 && liczba<=12)punkty[i]=punkty[i]+3*zakl[ktora_runda][i][j].kasa;
		}
	else if(zakl[ktora_runda][i][j].tabp[0]==2){
		if(liczba>=13 && liczba<=24)punkty[i]=punkty[i]+3*zakl[ktora_runda][i][j].kasa;
		}
	else if(zakl[ktora_runda][i][j].tabp[0]==3){
		if(liczba>=25 && liczba<=36)punkty[i]=punkty[i]+3*zakl[ktora_runda][i][j].kasa;
		}
}	

void obstaw_zaklad5( zaklad zakl[][max_graczy][max_zakladow], int j, int i, int ktora_runda, int punkty[], char zaklady_specjalnex[]){ //obstawianie zakladu typu Even or Odd
	...
	zakl[ktora_runda][i][j].nr_zak=5; //wpisanie informacji o obstawionym zak³adzie do tablicy struktur ¿eby pozniej mo¿na by³o srawdziæ czy gracz wygra³
	zakl[ktora_runda][i][j].tabp[0]=x; //
...
void wynik_zaklad_typ5(zaklad zakl[][max_graczy][max_zakladow], int i, int j, int liczba, int ktora_runda, int punkty[] ){ //sprawdzanie czy liczba obstawiona przez gracza w zakladzie Even or Odd zosta³a wylosowana
	if(zakl[ktora_runda][i][j].tabp[0]==1){
		if(liczba%2==0)punkty[i]=punkty[i]+2*zakl[ktora_runda][i][j].kasa;
			}
	else if(zakl[ktora_runda][i][j].tabp[0]==2){
		if(liczba%2!=0)punkty[i]=punkty[i]+2*zakl[ktora_runda][i][j].kasa;
			}
...
void obstaw_zaklad6( zaklad zakl[][max_graczy][max_zakladow], int j, int i, int ktora_runda, int punkty[], char zaklady_specjalnex[]){ //obstawianie zakladu typu 1st 18 or 2nd 18
	...
	zakl[ktora_runda][i][j].nr_zak=6; //wpisanie informacji o obstawionym zak³adzie do tablicy struktur ¿eby pozniej mo¿na by³o srawdziæ czy gracz wygra³
	zakl[ktora_runda][i][j].tabp[0]=x;//
...
void wynik_zaklad_typ6(zaklad zakl[][max_graczy][max_zakladow], int i, int j, int liczba, int ktora_runda, int punkty[] ){ //sprawdzanie czy liczba obstawiona przez gracza w zakladzie 1st 18 or 2nd 18 zosta³a wylosowana
	if(zakl[ktora_runda][i][j].tabp[0]==1){
		if(liczba>=1 && liczba<=18)punkty[i]=punkty[i]+2*zakl[ktora_runda][i][j].kasa;
			}
	else if(zakl[ktora_runda][i][j].tabp[0]==2){
		if(liczba>=19 && liczba<=36)punkty[i]=punkty[i]+2*zakl[ktora_runda][i][j].kasa;
		}
 }	

void obstaw_zaklad7( zaklad zakl[][max_graczy][max_zakladow], int j, int i, int ktora_runda, int punkty[], int tab[], char zaklady_specjalnex[]){ //obstawianie zakladu typu Red or Black
	...
	zakl[ktora_runda][i][j].nr_zak=7; //wpisanie informacji o obstawionym zak³adzie do tablicy struktur ¿eby pozniej mo¿na by³o srawdziæ czy gracz wygra³
	zakl[ktora_runda][i][j].tabp[0]=x; //
...
void wynik_zaklad_typ7(zaklad zakl[][max_graczy][max_zakladow], int i, int j, int liczba, int ktora_runda, int punkty[], int tab[] ){//sprawdzanie czy liczba obstawiona przez gracza w zakladzie Red or Black zosta³a wylosowana
	if(zakl[ktora_runda][i][j].tabp[0]==tab[liczba]){
	punkty[i]=punkty[i]+2*zakl[ktora_runda][i][j].kasa;
	}
}	
...
int main(){
	...
	while(punkty_0==false){ //gra toczy sie dopóki jeden z graczy nie bedzie mia³ zero punktów po zakoñczeniu rundy
		...
		zaklad zakl[max_rund][max_graczy][max_zakladow]; //tablica struktur dla jednego zakladu jednego gracza w jednej rundzie
		for(int i=0; i<ile_graczy; i++){ //obstawianie zak³adów przez kolejnych graczy
			
			funkcja_zerujaca(straightx, splitx1, splitx2 , cornerx, zaklady_specjalnex);
			rysuj(straightx, splitx1, splitx2, cornerx, zaklady_specjalnex); 
			
			cout<<"Zaklady obstawia gracz numer "<<i+1<<endl<<endl;
			bool ile_rund=true;
			int ile_zakladow=0; //ile zak³adów obstawi³ gracz w swojej turze
			system("pause");
			system("cls");
			...
			while(ile_rund==true){ //gracz wykonuje tyle zak³adów ile chce
				...
				switch( zaklad_numer ){ //gracz wybiera numer zak³adu który chcia³by obstawiæ
					case 1:	obstaw_zaklad1(zakl, ile_zakladow, i, ktora_runda, punkty, straightx);
							ile_zakladow=ile_zakladow+1;	
						break;
		    		case 2: obstaw_zaklad2(zakl, ile_zakladow, i, ktora_runda, punkty, splitx1, splitx2);
							ile_zakladow=ile_zakladow+1;	
						break;	
		    		case 3:	obstaw_zaklad3(zakl, ile_zakladow, i, ktora_runda, punkty, cornerx);
							ile_zakladow=ile_zakladow+1;		
		    			break;
		    		case 4: obstaw_zaklad4(zakl, ile_zakladow, i, ktora_runda, punkty, zaklady_specjalnex);
							ile_zakladow=ile_zakladow+1;	
						break;
					case 5: obstaw_zaklad5(zakl, ile_zakladow, i, ktora_runda, punkty, zaklady_specjalnex);
							ile_zakladow=ile_zakladow+1;	
						break;
					case 6: obstaw_zaklad6(zakl, ile_zakladow, i, ktora_runda, punkty, zaklady_specjalnex);
							ile_zakladow=ile_zakladow+1;	
						break;
					case 7: obstaw_zaklad7(zakl, ile_zakladow, i, ktora_runda, punkty, pola, zaklady_specjalnex);
							ile_zakladow=ile_zakladow+1;	
						break;
				...
				system("cls");
				rysuj(straightx, splitx1, splitx2, cornerx, zaklady_specjalnex); //rysowanie sto³u z obstawionymi zak³adami
			...
			if(ile_zakladow==0)cout<<"Nie postawiles zandych zakladow."<<endl;
			
			for(int i1=0; i1<ile_zakladow; i1++){ //wypisanie zak³adów postawionych przez gracza
				cout<<"Zaklad numer "<<i1+1<<" - ";
				switch(zakl[ktora_runda][i][i1].nr_zak){
					case 1:	cout<<"Straight Up | Liczba - "<<zakl[ktora_runda][i][i1].tabp[0]<<"| Zetony: "<<zakl[ktora_runda][i][i1].kasa<<endl;						
						break;
			    	case 2:	cout<<"Split Bet | Liczby - "<<zakl[ktora_runda][i][i1].tabp[0]<<" , "<<zakl[ktora_runda][i][i1].tabp[1]<<"| Zetony: "<<zakl[ktora_runda][i][i1].kasa<<endl;	
						break;
			    	case 3:	cout<<"Corner Bet | Liczby- "<<zakl[ktora_runda][i][i1].tabp[0]<<" , "<<zakl[ktora_runda][i][i1].tabp[1]<<" , "<<zakl[ktora_runda][i][i1].tabp[2]<<" , "<<zakl[ktora_runda][i][i1].tabp[3];
							cout<<"| Zetony: "<<zakl[ktora_runda][i][i1].kasa<<endl;	
			    		break;
			    	case 4:	cout<<"Dozen Bet | Przedzial - ";
						if(zakl[ktora_runda][i][i1].tabp[0]==1) cout<<"(1;12) |";
						else if(zakl[ktora_runda][i][i1].tabp[0]==2) cout<<"(13;24) |";
						else cout<<"(25;36) |";	
						cout<<"Zetony: "<<zakl[ktora_runda][i][i1].kasa<<endl;
						break;
					case 5:	cout<<"Even, Odd | Liczby - ";
						if(zakl[ktora_runda][i][i1].tabp[0]==1) cout<<"Parzyste |";
						else cout<<"Nieparzyste |";
						cout<<"Zetony: "<<zakl[ktora_runda][i][i1].kasa<<endl;	
						break;
					case 6:	cout<<"1st 18, 2nd 18 | Przedzial - ";
						if(zakl[ktora_runda][i][i1].tabp[0]==1) cout<<"1st 18 |";
						else cout<<"2nd 18 |";
						cout<<"Zetony: "<<zakl[ktora_runda][i][i1].kasa<<endl;	
						break;
					case 7:	cout<<"Red, Black| Kolor - ";
						if(zakl[ktora_runda][i][i1].tabp[0]==1) cout<<"Red |";
						else cout<<"Black |"; 
						cout<<"Zetony: "<<zakl[ktora_runda][i][i1].kasa<<endl;	
						break;
			    }
		...
		funkcja_zerujaca(straightx, splitx1, splitx2, cornerx, zaklady_specjalnex); //usuniêcie obstawionych zak³adów ze sto³u
		straightx[liczba-1]='X'; //zaznaczenie X na stole wylosowanej liczby
		if(liczba==0){           //
			straightx[36]='X';   //
		}                        //
		rysuj(straightx, splitx1, splitx2, cornerx, zaklady_specjalnex); //rysowanie sto³u z zaznaczeniem wylosowanej liczby

		for(int i=0; i<ile_graczy; i++){ //dodanie wygranej liczby ¿etonów do konta poszczególnych graczy
			for(int j=0; j<max_zakladow; j++){
				switch(zakl[ktora_runda][i][j].nr_zak){
					case 1: wynik_zaklad_typ1(zakl, i, j, liczba, ktora_runda,  punkty );
						break;
					case 2: wynik_zaklad_typ2(zakl, i, j, liczba, ktora_runda, punkty );
						break;
					case 3:	wynik_zaklad_typ3(zakl, i, j, liczba, ktora_runda, punkty );
						break;
					case 4: wynik_zaklad_typ4(zakl, i, j, liczba, ktora_runda, punkty );
						break;
					case 5: wynik_zaklad_typ5(zakl, i, j, liczba, ktora_runda, punkty );
						break;
					case 6: wynik_zaklad_typ6(zakl, i, j, liczba, ktora_runda, punkty );
						break;
					case 7: wynik_zaklad_typ7(zakl, i, j, liczba, ktora_runda, punkty, pola );
						break;
				}
			}
		}
	...
	for(int i=0; i<ile_graczy; i++){ //wypisanie ostateczne punktacji
